...
def uaku():
	try:
		...
		a = requests.get('https://github.com/xaikosiam/Siam-Approval.txt/blob/main/Siam143-approval.txt').text
		ua = open('.bbnew.txt', 'w')
...
\x1b[1;93m ║𒁍[✔] TOOLS     :      FILL CLONING    ║
...
def notice():
	...
	print('\x1b[0;92mADMIN WHATSAPP\x1aRR_Xaiko')
	time.sleep(2)
	os.system('am start https://wa.me/+8801319913395')
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/K41NG/text/main/Dis').text
			if basesplit in plr:
				key = basesplit
				stat = '\x1b[0;92mPAID'
				FY = '\x1b[0;93m'
		...
		if _____cowok__pink_____ in ('1',):
			if basesplit in plr:
				...
				os.system('am start https://wa.me/+880131993395')
			elif basesplit in ('3',):
				print('=>EXIT DONE ')
				exit()
			else:
